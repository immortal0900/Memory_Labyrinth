# DeepEval NPC 페르소나 평가 시스템 구현 완료

## 구현 완료 항목

### ✅ 1. 디렉토리 구조 설정
- `src/tests/persona_eval/` 디렉토리 생성
...
| **RoleAdherence** | AI 정체성 노출, 메타 인지, 세계관 일관성 | 90% |
| **KnowledgeBoundary** | 현대 지식, 해금 안된 기억, 시대 착오 | 80% |

### ✅ 4. NPC API 클라이언트 (`npc_client.py`)
- 비동기 HTTP 클라이언트 (httpx)
- 로그인, 히로인 대화, 대현자 대화 메서드
- 세션 조회 기능
- 컨텍스트 매니저 지원
...
- `lupames_questions.json` (루파메스 - 열정적인 늑대족)
- `roco_questions.json` (로코 - 소심한 하프 드워프)
...
- `type`: 질문 유형
- `turns`: 대화 턴 (단일/멀티턴)
- `persona_context`: 테스트 대상 페르소나 요소
...
python -m src.tests.persona_eval.question_generator
```
...
3. **대시보드**: DeepEval 결과를 시각화하는 웹 대시보드
