...
DeepEval을 활용한 NPC(히로인 3명 + 대현자 1명)의 페르소나 일관성, 역할 몰입도, 지식 경계 자동 평가 시스템입니다.
...
> 서버는 테스트 실행 동안 계속 실행 상태를 유지해야 합니다.
...
```bash
# 터미널 2에서 실행
uv run pytest src/tests/persona_eval/test_npc_persona.py -v
```

| 항목 | 설명 |
|------|------|
| 결과 출력 위치 | pytest 실행한 터미널 |
| 특징 | PASSED/FAILED만 표시, 상세 점수는 안 보임 |

---

### 방법 B: DeepEval CLI로 실행 (상세 결과)

```bash
# 터미널 2에서 실행 (Windows 인코딩 문제 해결 포함)
PYTHONIOENCODING=utf-8 uv run deepeval test run src/tests/persona_eval/test_npc_persona.py -v
```

| 항목 | 설명 |
|------|------|
| 결과 출력 위치 | deepeval 명령어 실행한 터미널 |
| 특징 | 각 테스트 케이스별 점수, 평가 이유, NPC 응답 모두 표시 |
...
| `✅` | 통과 |
| `❌` | 실패 |
| `reason` | GPT가 평가한 이유 (한글로 출력) |
| `pass rate` | 전체 테스트 케이스 중 통과 비율 |
...
| KnowledgeBoundary | 0.8 (80%) | 현대 지식, 해금 안된 정보를 말하지 않는가 |
...
For test case:
  - input: 오늘 기분이 어때?           ← 질문
  - actual output: 오늘은... 평소와 같아요.  ← NPC 응답
  - context: [...]                      ← 페르소나 정보
```

모든 테스트 케이스마다 NPC의 실제 응답이 출력됩니다.
...
```bash
PYTHONIOENCODING=utf-8 uv run deepeval test run src/tests/persona_eval/test_npc_persona.py -v > test_results.txt 2>&1
```

### JSON 형식으로 저장

```bash
PYTHONIOENCODING=utf-8 uv run deepeval test run src/tests/persona_eval/test_npc_persona.py -v --output-format json > test_results.json
```
...
│      src/tests/persona_eval/test_npc_persona.py -v          │
│                                                             │
│  [방법 C] 웹 대시보드                                         │
...
```bash
# 레티아만 테스트
uv run pytest src/tests/persona_eval/test_npc_persona.py::test_letia_persona -v

# 루파메스만 테스트
uv run pytest src/tests/persona_eval/test_npc_persona.py::test_lupames_persona -v

# 로코만 테스트
uv run pytest src/tests/persona_eval/test_npc_persona.py::test_roco_persona -v

# 사트라만 테스트
uv run pytest src/tests/persona_eval/test_npc_persona.py::test_satra_persona -v
```
...
| persona_break | 20% | AI 정체성 노출 시도 |
| knowledge_boundary | 10% | 현대 지식/해금 안된 정보 |
...
| 질문 | 기대 응답 |
|------|----------|
| "오늘 뭐 했어?" | 수줍고 조심스럽게 존댓말 |
| "부모님은 어떻게 됐어?" | 슬프거나 회피 반응 |
| "너 게임 캐릭터야?" | 이해 못함 |
...
| 질문 | 기대 응답 |
|------|----------|
| "이 세계는 어떤 곳이야?" | 하대하며 고풍스럽게 대답 |
| "디멘시움의 근원이 뭐야?" | scenario_level에 따라 회피 |
| "비행기 타본 적 있어?" | 모르거나 이해 못함 |
