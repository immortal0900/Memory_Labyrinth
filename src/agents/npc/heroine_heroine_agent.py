...
# 페르소나 YAML 파일 경로
PERSONA_PATH = Path(__file__).parent.parent.parent / "data" / "persona" / "heroine_persona.yaml"
...
class HeroineHeroineAgent:
    ...
    def __init__(self, model_name: str = "gpt-4o-mini"):
        ...
        self.streaming_llm = init_chat_model(model=model_name, temperature=0.8, streaming=True)
    ...
    def _get_relationship(self, heroine1_id: int, heroine2_id: int) -> str:
        ...
        """
        relationships = {
            (1, 2): "라이벌이자 동료. 레티아는 루파메스를 시끄럽지만 믿을 수 있다고 생각함",
...
선택한 상황과 함께 2-3문장으로 구체적인 상황을 설명하세요."""
    ...
    def _build_conversation_prompt(
        ...
        honorific1 = "존댓말" if persona1.get('speech_style', {}).get('honorific', False) else "반말"
        honorific2 = "존댓말" if persona2.get('speech_style', {}).get('honorific', False) else "반말"
...
JSON 배열로 출력하세요:
[
    {{"speaker_id": {heroine1_id}, "speaker_name": "{name1}", "text": "대사", "emotion": "감정"}},
    {{"speaker_id": {heroine2_id}, "speaker_name": "{name2}", "text": "대사", "emotion": "감정"}},
    ...
]"""
