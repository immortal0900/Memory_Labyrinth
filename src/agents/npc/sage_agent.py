...
# 페르소나 YAML 파일 경로
PERSONA_PATH = Path(__file__).parent.parent.parent / "prompts" / "prompt_type" / "npc" / "sage_persona.yaml"
...
반드시 general, worldview_inquiry, personal_inquiry, player_inquiry 중 하나만 출력하세요."""
        ...
        # 유효하지 않으면 기본값
        valid_intents = ["general", "worldview_inquiry", "personal_inquiry", "player_inquiry"]
...
{output_format}"""
    ...
    async def _post_process_node(self, state: SageState) -> dict:
        ...
        return {
            "info_revealed": state.get("info_revealed", False)
        }
