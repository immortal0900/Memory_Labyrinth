...
class RedisManager:
    ...
    def save_session(self, player_id: int, npc_id: int, session_data: Dict[str, Any]) -> None:
        """Redis에 세션 저장
    ...
    def update_session(self, player_id: int, npc_id: int, updates: Dict[str, Any]) -> Dict[str, Any]:
        """세션 부분 업데이트
    ...
    def add_conversation(self, player_id: int, npc_id: int, role: str, content: str) -> None:
        """대화 내용 추가
    ...
    def start_npc_conversation(self, player_id: int, npc1_id: int, npc2_id: int) -> None:
        """NPC간 대화 시작 등록
